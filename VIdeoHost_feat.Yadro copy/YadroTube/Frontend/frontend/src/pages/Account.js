...
const Account = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [userResponse, videosResponse] = await Promise.all([
          axios.get('http://212.164.112.171:8000/api/auth/me/', {
            headers: {
              Authorization: `Bearer ${accessToken}` // Используем полученный токен
            }
          }),
          axios.get('http://212.164.112.171:8000/api/videos/user/', {
            headers: {
              Authorization: `Bearer ${accessToken}` // Используем полученный токен
            }
          })
  ...
  const handleDeleteVideo = async (videoId) => {
    ...
    try {
      await axios.delete(`http://212.164.112.171:8000/api/videos/${videoId}/`, {
        headers: {
          Authorization: `Bearer ${accessToken}` // Используем полученный токен
        }
      });
